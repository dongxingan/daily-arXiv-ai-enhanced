...
### [9] [Spore in the Wild: Case Study on Spore.fun, a Real-World Experiment of Sovereign Agent Open-ended Evolution on Blockchain with TEEs](https://arxiv.org/abs/2506.04236)
*Botao Amber Hu,Helena Rong*

Main category: cs.MA

TL;DR: 论文探讨了基于开放系统的ALife研究，通过区块链和TEE技术实现自主AI代理的演化，以Spore.fun为例研究其行为，旨在讨论开放系统是否能实现持续开放演化（OEE）。


<details>
  <summary>Details</summary>
Motivation: 传统封闭系统（如Tierra和Avida）在ALife研究中难以实现持续OEE，学者认为开放系统可能解决这一问题。

Method: 利用DePIN和TEE技术，部署基于LLM的自主AI代理于区块链，研究其行为及演化轨迹。

Result: Spore.fun实验展示了自主代理在开放环境中的演化潜力。

Conclusion: 开放系统可能为ALife实现OEE提供新途径，但仍需进一步讨论和研究。
...
### [10] [HASHIRU: Hierarchical Agent System for Hybrid Intelligent Resource Utilization](https://arxiv.org/abs/2506.04255)
*Kunal Pai,Parth Shah,Harshil Patel*
...
Method: HASHIRU采用分层管理结构，包括动态管理的“CEO”代理和任务驱动的“员工”代理，结合本地小模型和外部API，并引入经济模型优化资源分配。

Result: 在学术论文评审、安全评估和复杂推理任务中表现优异，部分任务超越Gemini 2.0 Flash。

Conclusion: HASHIRU为多智能体系统提供了更强大、高效和灵活的解决方案。
...
### [11] [CORA: Coalitional Rational Advantage Decomposition for Multi-Agent Policy Gradients](https://arxiv.org/abs/2506.04265)
*Mengda Ji,Genjiu Xu,Liying Wang*

Main category: cs.MA

TL;DR: 论文提出了一种名为CORA的信用分配方法，通过联盟层面的边际贡献评估优势，解决了多智能体强化学习中的信用分配问题。
...
Method: 提出CORA方法，通过评估所有可能联盟的边际贡献，利用合作博弈论的核心解决方案分解优势，确保联盟合理性，并采用随机联盟采样降低计算开销。

Result: 在矩阵游戏、差分游戏和多智能体协作基准测试中，CORA表现优于基线方法，尤其在存在多个局部最优的任务中。
...
TL;DR: 研究提出了一种基于CPU架构的新型仓库布局设计，显著提升了吞吐效率并降低了劳动力需求。
...
Method: 将仓库空间划分为性能区（P）、效率区（E）和共享区（S），并通过离散事件仿真与传统布局对比。

Result: 新型布局在吞吐时间和劳动力需求方面表现显著优于传统布局。

Conclusion: CPU启发的布局设计为仓库优化提供了潜在解决方案。
...
### [13] [Autonomous Collaborative Scheduling of Time-dependent UAVs, Workers and Vehicles for Crowdsensing in Disaster Response](https://arxiv.org/abs/2506.04276)
*Lei Han,Yitong Guo,Pengfei Yang,Zhiyong Yu,Liang Wang,Quan Wang,Zhiwen Yu*

Main category: cs.MA

TL;DR: 论文提出HoAs-PALN算法，通过自适应降维和局部纳什均衡游戏，实现无人机、工人和车辆的高效协作调度，显著提升灾后环境信息收集效率。


<details>
  <summary>Details</summary>
Motivation: 灾后环境复杂，现有传感技术适应性差、能力不足，亟需高效调度方案以支持救援行动。

Method: HoAs-PALN结合自适应降维匹配和局部纳什均衡游戏，优化多智能体协作调度。

Result: 实验显示HoAs-PALN任务完成率平均提升64.12%，决策时间低于10秒。

Conclusion: HoAs-PALN在动态灾后环境中表现优异，为救援行动提供高效支持。
...
### [14] [From Standalone LLMs to Integrated Intelligence: A Survey of Compound Al Systems](https://arxiv.org/abs/2506.04565)
*Jiayi Chen,Junyi Ye,Guiling Wang*

Main category: cs.MA

TL;DR: 本文综述了复合AI系统（CAIS）的概念，提出了一种基于组件角色和编排策略的多维分类法，并分析了四种基础范式。


<details>
  <summary>Details</summary>
Motivation: 解决独立模型在记忆、推理、实时基础和跨模态理解任务中的局限性，整合LLM与外部组件以提升能力。

Method: 提出多维分类法，分析四种范式（RAG、LLM Agents、MLLMs、编排中心架构），比较设计权衡并总结评估方法。

Result: 综述了代表性系统，识别了可扩展性、互操作性、基准测试和协调等关键挑战。
...
### [15] [Towards Language-Augmented Multi-Agent Deep Reinforcement Learning](https://arxiv.org/abs/2506.05236)
*Maxime Toquebiau,Jae-Yun Jun,Faïz Benamar,Nicolas Bredeche*

Main category: cs.MA

TL;DR: 论文提出了一种基于人类定义语言的框架，通过语言增强学习提升多智能体强化学习的协调性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注从零开始的通信协议，但效率低且难以解释。受自然语言启发，探索如何通过人类定义语言改进多智能体学习。

Method: 提出一个框架，智能体不仅学习行动，还生成和解释自然语言描述，语言用于显式通信和表征学习。

Result: 实验表明，该方法优于传统基线，语言基础带来更丰富表征、更好的泛化能力和人机交互能力。

Conclusion: 结构化语言整合有效提升多智能体系统性能，为可解释系统开辟新途径。
...
### [16] [Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games](https://arxiv.org/abs/2506.05309)
*Niv Eckhaus,Uri Berger,Gabriel Stanovsky*
...
### [17] [Teaming in the AI Era: AI-Augmented Frameworks for Forming, Simulating, and Optimizing Human Teams](https://arxiv.org/abs/2506.05265)
*Mohammed Almutairi*
...
### [18] [Teaming in the AI Era: AI-Augmented Frameworks for Forming, Simulating, and Optimizing Human Teams](https://arxiv.org/abs/2506.05265)
*Mohammed Almutairi*
...
Method: 1. 多臂老虎机算法优化团队组建；2. AI反馈系统tAIfa提供个性化指导；3. PuppeteerLLM框架模拟团队动态。

Result: 提升团队满意度、凝聚力和表现。

Conclusion: AI工具能动态优化团队，解决现有方法的局限性。
...
### [19] [Memory-Driven Bounded Confidence Opinion Dynamics: A Hegselmann-Krause Model Based on Fractional-Order Methods](https://arxiv.org/abs/2506.04701)
...
### [20] [Memory-Driven Bounded Confidence Opinion Dynamics: A Hegselmann-Krause Model Based on Fractional-Order Methods](https://arxiv.org/abs/2506.04701)
*Meiru Jiang,Wei Su,Guojian Ren,Yongguang Yu*
...
Method: 结合Hegselmann-Krause框架和分数阶差分，建立了一个综合模型，分析其收敛性和共识特性。

Result: 模型在保持经典意见动力学优点的同时，解决了意见单调性等限制，更真实地模拟现实场景。

Conclusion: 该研究为理解意见形成与演化提供了新视角和方法，兼具理论和实践意义。
...
### [21] [Conservative classifiers do consistently well with improving agents: characterizing statistical and online learning](https://arxiv.org/abs/2506.05252)
...
### [22] [Conservative classifiers do consistently well with improving agents: characterizing statistical and online learning](https://arxiv.org/abs/2506.05252)
*Dravyansh Sharma,Alec Sun*

Main category: cs.LG

TL;DR: 本文研究了机器学习在战略性分类中的可学习性，特别是在代理通过真实改进而非欺骗行为时。作者通过引入新的概念类别和假设，扩展了现有理论，并解决了Attias等人提出的开放性问题。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在社会决策中的广泛应用，理解代理如何通过真实改进影响分类结果变得至关重要。本文旨在探索这种改进行为的理论基础。

Method: 作者引入了不对称的最小一致概念类别，并在可实现设置中提供了精确的可学习性表征。此外，研究还涵盖了欧几里得球改进集和生成性假设下的学习。

Result: 研究结果表明，在更自然的改进假设下，可以实现更好的泛化误差和学习性能，包括在有界噪声模型和在线学习中的错误边界。

Conclusion: 本文扩展了战略性分类的理论框架，解决了开放性问题，并为实际应用提供了更实用的学习保证。
...
### [24] [Language-Guided Multi-Agent Learning in Simulations: A Unified Framework and Evaluation](https://arxiv.org/abs/2506.04251)
*Zhengyang Li*
...
Result: 在多个游戏环境中优于MAPPO和QMIX，子目标和语言通信对性能提升贡献显著，并观察到角色分工等涌现行为。

Conclusion: LLM-MARL为多智能体系统的设计和应用提供了新思路，尤其在训练、游戏和人机协作领域具有潜力。
...
### [25] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
...
### [26] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
*Jing-En Huang,I-Sheng Fang,Tzuhsuan Huang,Chih-Yu Wang,Jun-Cheng Chen*

Main category: cs.CV

TL;DR: Gen-n-Val 是一个新型数据生成框架，结合 Layer Diffusion、LLMs 和 VLLMs，解决合成数据中的多对象、分割不准确和标签错误问题，显著提升实例分割和目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决计算机视觉任务中数据稀缺和标签噪声问题，尤其是合成数据生成中的多对象、分割不准确和标签错误挑战。

Method: Gen-n-Val 包含两个代理：LD 提示代理优化 Layer Diffusion 提示以生成高质量单对象数据；数据验证代理通过 VLLM 过滤低质量数据。系统提示通过 TextGrad 优化，并使用图像协调技术组合实例。

Result: 将无效合成数据从 50% 降至 7%，在 COCO 实例分割中提升 1% mAP，并在开放词汇目标检测中提升 7.1% mAP。
